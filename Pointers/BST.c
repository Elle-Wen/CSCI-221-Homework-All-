/*#include <stdio.h>
#include <stdlib.h>
#include "BST.h"
//store a node
struct Node{
    int data;
    struct Node* left;
    struct Node* right;
}; 
// free a BST
struct Node* makeEmpty(struct Node* root){
    makeEmpty(root->left);
    makeEmpty(root->right);
    free(root);
    return NULL;
}
//insert a value to BST
struct Node* insert(struct Node* root, int x){
    if(root == NULL){
        root = malloc(sizeof(struct Node));
        root->data = x;
        root->left = root->right = NULL;
    }
    else if(x < root->data){
        root->left = insert(root->left, x);
    }
    else if(x > root->data){
        root->right = insert(root->right, x);
    }
    return root;
}
void inorder(struct Node* root){
    inorder(root->left);
    printf("%d ", root->data);
    inorder(root->right);
}
struct Node* findMin(struct Node* root){
    if(root == NULL)
    	return NULL;
    else if(root->left == NULL)
    	return root;
    else
    	return findMin(root->left);
}
// delete a value
struct Node* del(struct Node* root, int x){
    struct Node* temp;
    if(x < root->data){
        root->left = del(root->left, x);
    }
    else if(x > root->data){
        root->right = del(root->right, x);
    }
    else if(root->left && root->right){
        temp = findMin(root->right);
        root->data = temp->data;
        root->right = delete(root->right, root->data);
    }
    else{
        temp = root;
        if(root->left == NULL)
            root = root->right;
        else if(root->right == NULL)
            root = root->left;
        free(temp);
    }
    return x;
}
// take an array and convert to a BST
struct Node* buildTree(struct Node* root, int* array, int start, int end){
        if(start > end)
            return NULL;
        int mid = start + (end - start) / 2;
        root = Node(array[mid]);
        root -> left = buildTree(root, array, start, mid - 1);
        root -> right = buildTree(root, array, mid + 1, end);
        return root;
    }
int main(){
    struct TreeNode *root;
    struct TreeNode *temp;
    root = NULL;
    root = insert(root, 15);
    root = insert(root, 10);
    root = insert(root, 25);
    root = insert(root, 7);
    printf("Height: %d\n", findHeight(root));
    inorder(root); printf("\n");
    root = delete(root, 10);
    inorder(root); printf("\n");
    root = delete(root, 25);
    inorder(root); printf("\n");
    root = makeEmpty(root);
    int array[4] = {1,2,3,4};
    root = buildTree(root, array, 2, 3);
    return 0;
*/


#include<stdio.h>
#include<stdlib.h>
// I worked together with a friend at UVA, but wrote my own code. 
// make a node
struct TreeNode{
    int data;
    struct TreeNode* left;
    struct TreeNode* right;
};
// free the tree
struct TreeNode* makeEmpty(struct TreeNode* root){
    if(root != NULL){
        makeEmpty(root->left);
        makeEmpty(root->right);
        free(root);
    }
    return NULL;
}
// insert the node
struct TreeNode* insert(struct TreeNode* root, int x){
    if(root == NULL){
        root = malloc(sizeof(struct TreeNode));
        root->data = x;
        root->left = root->right = NULL;
    }
    else if(x < root->data){
        root->left = insert(root->left, x);
    }
    else if(x > root->data){
        root->right = insert(root->right, x);
    }
    return root;
}
// a helper struc for delete
struct TreeNode* findMin(struct TreeNode* root){
    if(root == NULL)
    	return NULL;
    else if(root->left == NULL)
    	return root;
    else
    	return findMin(root->left);
}
// might be useful in future 
/*
struct TreeNode* findMax(struct TreeNode* root){
    if(root == NULL)
    	return NULL;
    else if(root->right == NULL)
    	return root;
    else
    	return findMax(root->right);
}
*/
// a helper func
/*
struct TreeNode* find(struct TreeNode* root, int x){
    if(root == NULL)
        return NULL;
    else if(x < root->data)
        return find(root->left, x);
    else if(x > root->data)
        return find(root->right, x);
    else
        return root;
}
*/
// to print and test
int findHeight(struct TreeNode* root){
    int lefth, righth;
    if(root == NULL)
        return -1;
    lefth = findHeight(root->left);
    righth = findHeight(root->right);
    return (lefth > righth ? lefth : righth)+1;
}
//delete a value
struct TreeNode* delete(struct TreeNode* root, int x){
    struct TreeNode* temp;
    if(root == NULL)
        return NULL;
    else if(x < root->data)
        root->left = delete(root->left, x);
    else if(x > root->data)
        root->right = delete(root->right, x);
    else if(root->left && root->right){
        temp = findMin(root->right);
        root->data = temp->data;
        root->right = delete(root->right, root->data);
    }
    else{
        temp = root;
        if(root->left == NULL)
            root = root->right;
        else if(root->right == NULL)
            root = root->left;
        free(temp);
    }
    printf("%d",x);
    return root;
}
// to make the tree in order!
void inorder(struct TreeNode* root){
    if(root == NULL)
        return;
    inorder(root->left);
    printf("%d ", root->data);
    inorder(root->right);
}
// take an array and convert to a BST
struct TreeNode *gen_tree(int *nums, int start, int end) {
    struct TreeNode *node;
    int mid;
    if (start > end) return NULL;
    node = malloc(sizeof(struct TreeNode));
    //assert(node);
    mid = start + (end - start) / 2;
    node->data = nums[mid];
    node->left = gen_tree(nums, start, mid - 1);  // sub tree on left
    node->right = gen_tree(nums, mid + 1, end);  // sub tree on right
    return node;
}
int main(){
    struct TreeNode *root;
    struct TreeNode *temp;
    root = NULL;
    root = insert(root, 15);
    root = insert(root, 10);
    root = insert(root, 13);
    root = insert(root, 18);
    root = insert(root, 30);
    root = insert(root, 3);
    root = insert(root, 8);
    root = insert(root, 16);
    root = insert(root, 22);
    root = insert(root, 35);
    inorder(root); printf("\n");
    temp = findMin(root);
    printf("Min Element: %d\n", temp->data);
    root = delete(root, 8);
    root = delete(root, 16);
    inorder(root); printf("\n");
    root = makeEmpty(root);
    return 0;
}



